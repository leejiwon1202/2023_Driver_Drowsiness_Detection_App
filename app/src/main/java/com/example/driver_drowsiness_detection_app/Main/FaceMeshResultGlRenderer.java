...
public class FaceMeshResultGlRenderer implements ResultGlRenderer<FaceMeshResult> {
  ...
  public void renderResult(FaceMeshResult result, float[] projectionMatrix) {
    ...
    for (int i = 0; i < numFaces; ++i) {
      //drawLandmarks(result.multiFaceLandmarks().get(i).getLandmarkList(),
      //    FaceMeshConnections.FACEMESH_TESSELATION, TESSELATION_COLOR, TESSELATION_THICKNESS);
    }
  }

  private void drawLandmarks(List<NormalizedLandmark> faceLandmarkList, ImmutableSet<FaceMeshConnections.Connection> connections, float[] colorArray, int thickness) {
